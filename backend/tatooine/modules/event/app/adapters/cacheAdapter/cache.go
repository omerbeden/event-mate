...
import (
	...
	"github.com/omerbeden/event-mate/backend/tatooine/modules/event/app/domain"
...
func GetEvent(id int, city string, cacher caching.Cache) (*model.Event, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cache get event: %w", err)
	}
	...
	if idx == -1 {
		return nil, domain.ErrCacheItemNotFound // nil dönülecek

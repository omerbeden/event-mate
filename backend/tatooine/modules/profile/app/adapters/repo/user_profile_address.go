...
func (r *userProfileAddressRepo) Insert(ctx context.Context, userId int64, address model.UserProfileAdress) error {
	q := `INSERT INTO user_profile_addresses (profile_id,city) Values($1,$2)`
	_, err := r.pool.Exec(ctx, q, userId, address.City)
	if err != nil {

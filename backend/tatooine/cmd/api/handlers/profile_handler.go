...
func GetUserProfile(service entrypoints.UserService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		profile, err := service.GetUserProfileById(ctx, id)
...
func GetCreatedActivities(service entrypoints.UserService) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		result, err := service.GetCreatedActivities(ctx, userId)
